...
export default async function ReviewPage({
  ...
  const configuredProviders = configuredRecallProviders();
  if (requestedTopicId) {
    const topic = await getTopic(db, requestedTopicId);
    if (!topic) notFound();
    return <ReviewSession initialTopics={[topic]} mode="single" configuredProviders={configuredProviders} />;
  }
  ...
  return <ReviewSession initialTopics={topics.filter((topic) => topic.reviewState.dueOn <= today).sort((a, b) => a.reviewState.dueOn.localeCompare(b.reviewState.dueOn))} configuredProviders={configuredProviders} />;
}
