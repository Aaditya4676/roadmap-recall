...
export function LoginForm({ configured }: { configured: boolean }) {
  ...
  return <form onSubmit={submit} className="grid gap-5"><label className="grid gap-1.5 font-semibold">Owner email<input type="email" autoComplete="email" required className="field font-normal" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" /></label>{error && <p role="alert" className="rounded-lg border border-[color:var(--danger)]/25 bg-[color:var(--danger)]/8 p-3 text-sm text-[var(--danger)]">{error}</p>}<button className="button-primary" data-liquid disabled={status === "sending"}><Send size={17} /> {status === "sending" ? "Sending…" : "Email me a magic link"}</button></form>;
}
