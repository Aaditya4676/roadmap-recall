...
export function CaptureTopic({ roadmapItemId, label = "Add what I learned", initialTitle = "" }: { roadmapItemId?: string; label?: string; initialTitle?: string }) {
  ...
  return (
    <>
      <button className={roadmapItemId ? "button-secondary !min-h-9" : "button-secondary"} data-liquid onClick={() => setOpen(true)}>
        <Plus size={17} /> {label}
      </button>
      {open && (
        <div className="modal-backdrop fixed inset-0 z-50 grid place-items-end sm:place-items-center sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
          <section role="dialog" aria-modal="true" aria-label="Capture learned topic" className="liquid-dialog max-h-[92vh] w-full overflow-y-auto rounded-t-xl p-6 sm:max-w-xl sm:rounded-[12px]" data-liquid>
