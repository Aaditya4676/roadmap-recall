...
export function NoteEditor({ topicId, initialMarkdown, initialRevision }: { topicId: string; initialMarkdown: string; initialRevision: number }) {
  ...
  return <section><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">My notes</h2><p className="flex items-center gap-1.5 text-xs text-[var(--muted)]">{status === "saved" ? <><Check size={13} /> Saved</> : status === "saving" ? <><Cloud size={13} /> Saving…</> : status === "offline" ? <><CloudOff size={13} /> Local draft kept; retry when online</> : status === "conflict" ? <><CloudOff size={13} /> Newer server revision found—copy your draft, then reload</> : "Unsaved changes"}</p></div><button className="button-secondary !min-h-9" data-liquid onClick={() => setEditing((value) => !value)}><Pencil size={15} /> {editing ? "Preview" : "Edit"}</button></div>{editing ? <><textarea className="field min-h-80 resize-y font-mono text-sm" value={markdown} onChange={(e) => change(e.target.value)} /><div className="mt-3 flex justify-end"><button className="button-primary !min-h-9" data-liquid disabled={status === "saved" || status === "saving"} onClick={() => save()}>Save now</button></div></> : <div className="reading-plane p-5 sm:p-6"><Markdown>{savedMarkdown || "_No personal notes yet. Choose Edit to add your explanation._"}</Markdown></div>}</section>;
}
