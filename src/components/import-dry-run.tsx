...
export function ImportDryRun() {
  ...
  return <section className="content-surface p-5 sm:p-6"><h2 className="text-xl font-bold">Validate a restore file</h2><p className="mt-1 text-sm text-[var(--muted)]">Dry-run validation checks schema, table counts, and orphaned note references without changing any row.</p><label className="button-secondary mt-4 cursor-pointer" data-liquid><FileCheck2 size={17} /> {busy ? "Inspecting…" : "Choose JSON backup"}<input className="sr-only" type="file" accept="application/json,.json" disabled={busy} onChange={(e) => check(e.target.files?.[0])} /></label>{status && <p role="status" className="mt-3 text-sm text-[var(--muted)]">{status}</p>}</section>;
}
