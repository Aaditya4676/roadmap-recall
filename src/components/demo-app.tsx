...
function DemoRoadmap({ topics, onSelect, onCapture }: { topics: StudyTopic[]; onSelect: (topic: StudyTopic) => void; onCapture: () => void }) {
  ...
  return <><PageHeading eyebrow="Learning library" title="A place for anything you learn" description="Organize a plan or add topics as you go. Only schedule material after you have actually studied it." action={<button className="button-primary" onClick={onCapture}><Plus size={18} /> Add a topic</button>} /><label className="relative mb-6 block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={18} /><span className="sr-only">Search topics</span><input className="field !pl-10" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search title or section…" /></label><TopicGroup label="Primary plan" topics={primary} onSelect={onSelect} /><TopicGroup label="Extension plan" topics={extension} onSelect={onSelect} /></>;
}
