...
const STORAGE_KEY = "roadmap-recall-demo-v1";
...
function Modal({ children, label, onClose }: { children: React.ReactNode; label: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-end bg-black/35 p-0 sm:place-items-center sm:p-5" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <section className="max-h-[92vh] w-full overflow-y-auto rounded-t-xl border border-[var(--border)] bg-[var(--surface)] p-5 sm:max-w-2xl sm:rounded-[10px] sm:p-7" role="dialog" aria-modal="true" aria-label={label}>
  ...
  return (
    <>
      ...
      <section className="card mt-6 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--border)] p-5 sm:p-6">
          ...
          <button className="button-primary" disabled={!due.length} onClick={onStart}><Brain size={18} /> Start review <ArrowRight size={17} /></button>
        </div>
      ...
      <details className="card mt-6 group">
        ...
        <div className="border-t border-[var(--border)] p-5"><div className="grid gap-2">{upcoming.map((topic) => <button key={topic.id} className="flex items-center justify-between gap-4 rounded-xl px-3 py-2 text-left hover:bg-[var(--subtle)]" onClick={() => onSelect(topic)}><span className="font-medium">{topic.title}</span><span className="shrink-0 text-xs text-[var(--muted)]">{humanDate(topic.reviewState.dueOn)}</span></button>)}</div></div>
      </details>
    </>
  );
}
...
function TopicRow({ topic, today, onClick }: { topic: StudyTopic; today: string; onClick: () => void }) {
  ...
  return <button onClick={onClick} className="flex w-full items-center gap-4 p-4 text-left hover:bg-[var(--subtle)] sm:px-6"><BookOpen size={18} className="shrink-0 text-[var(--muted)]" /><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{topic.title}</span><span className="block truncate text-xs text-[var(--muted)]">{topic.breadcrumb}</span></span>{topic.aiNote && <Sparkles size={16} className="shrink-0 text-[var(--accent)]" aria-label="AI notes available" />}<span className={`shrink-0 rounded-md px-2 py-1 text-xs font-bold ${overdue ? "bg-orange-100 text-orange-800 dark:bg-orange-950 dark:text-orange-200" : "bg-[var(--accent-soft)] text-[var(--accent)]"}`}>{overdue ? "Waiting" : "Today"}</span></button>;
}

function DemoRoadmap({ topics, onSelect, onCapture }: { topics: StudyTopic[]; onSelect: (topic: StudyTopic) => void; onCapture: () => void }) {
  const [query, setQuery] = useState("");
  const filtered = topics.filter((topic) => `${topic.title} ${topic.breadcrumb}`.toLowerCase().includes(query.toLowerCase()));
  const frontend = filtered.filter((topic) => topic.part === "frontend");
  const fullstack = filtered.filter((topic) => topic.part === "fullstack");
  return <><PageHeading eyebrow="Your catalog" title="Frontend first, full-stack clearly separated" description="The production catalog starts with all 795 roadmap items inactive. Activate only what you actually learn." action={<button className="button-primary" onClick={onCapture}><Plus size={18} /> Manual topic</button>} /><label className="relative mb-6 block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={18} /><span className="sr-only">Search topics</span><input className="field !pl-10" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search title or section…" /></label><TopicGroup label="Part I · Frontend engineering" topics={frontend} onSelect={onSelect} /><TopicGroup label="Part II · Full-stack extension" topics={fullstack} onSelect={onSelect} /></>;
}
...
function DemoProgress({ topics }: { topics: StudyTopic[] }) {
  ...
  return <><PageHeading eyebrow="Progress without pressure" title="Evidence of learning" description="This view values topics captured and recalls completed—not streaks or guilt." /><dl className="grid grid-cols-3 divide-x divide-[var(--border)] border-y border-[var(--border)]"><Metric value={topics.length} label="topics in active study" tone="accent" /><Metric value={reviews} label="reviews completed here" /><Metric value={ai} label="topics with optional AI notes" /></dl><section className="card mt-6 p-6"><h2 className="text-xl font-bold">Track balance</h2><div className="mt-6 grid gap-5"><ProgressLine label="Frontend" value={topics.filter((topic) => topic.part === "frontend").length} total={topics.length} /><ProgressLine label="Full-stack extension" value={topics.filter((topic) => topic.part === "fullstack").length} total={topics.length} /><ProgressLine label="Practice and projects" value={topics.filter((topic) => topic.kind !== "knowledge").length} total={topics.length} /></div></section></>;
}
...
function DemoSettings({ onReset }: { onReset: () => void }) {
  return <><PageHeading eyebrow="Demo preferences" title="Simple defaults, explicit choices" description="Production settings sync across devices. These demo controls only affect this browser." /><section className="card divide-y divide-[var(--border)]"><Setting title="Scheduler" copy="Fixed 1 day → 7 days → 30 days, then every 30 days"><span className="rounded-md border border-[var(--border)] px-2.5 py-1 text-sm font-semibold text-[var(--muted)]">Fixed rhythm</span></Setting><Setting title="Timezone" copy="Dates become due by owner calendar day, not by an arbitrary UTC midnight."><span className="text-sm font-semibold">Asia/Kolkata</span></Setting><Setting title="Reset sandbox" copy="Restore the ten curated demo topics and remove your local changes."><button className="button-secondary" onClick={onReset}><RotateCcw size={17} /> Reset demo</button></Setting></section></>;
}
...
function ReviewModal({ topics, onClose, onUpdate }: { topics: StudyTopic[]; onClose: () => void; onUpdate: (topic: StudyTopic) => void }) {
  ...
  return <Modal label={`Review ${topic.title}`} onClose={onClose}><div className="flex items-center justify-between gap-4"><p className="eyebrow">Topic {index + 1} of {due.length}</p><span className="text-xs text-[var(--muted)]">Recall before revealing</span></div><h2 className="mt-2 text-balance text-3xl font-bold">{topic.title}</h2><p className="mt-1 text-sm text-[var(--muted)]">{topic.breadcrumb}</p><label className="mt-6 grid gap-2 font-semibold">What can you explain from memory?<textarea autoFocus className="field min-h-32 resize-y font-normal" value={scratch} onChange={(e) => setScratch(e.target.value)} placeholder="Private scratchpad for this review…" /></label>{!revealed ? <button className="button-primary mt-5 w-full" onClick={() => setRevealed(true)}>Reveal my notes</button> : <><div className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--surface-strong)] p-5"><p className="eyebrow mb-3">My notes</p><Markdown>{topic.note.markdown || "_No notes yet._"}</Markdown></div>{topic.aiNote && !topic.aiNote.hidden && (!aiRevealed ? <button className="button-secondary mt-3 w-full" onClick={() => setAiRevealed(true)}><Sparkles size={17} /> Reveal separate AI notes</button> : <div className="mt-3 rounded-2xl border border-[var(--border)] bg-[var(--accent-soft)] p-5"><p className="eyebrow mb-3">AI notes · {topic.aiNote.model}</p><p>{topic.aiNote.document.summary}</p><ul className="mt-3 list-disc pl-5">{topic.aiNote.document.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul></div>)}<p className="mb-2 mt-6 text-center text-sm font-semibold">How well did you recall it?</p><div className="grid grid-cols-3 gap-2">{(["again", "hard", "good"] as const).map((rating) => <button key={rating} className={rating === "good" ? "button-primary !px-2 capitalize" : "button-secondary !px-2 capitalize"} onClick={() => rate(rating)}>{rating}</button>)}</div></> }</Modal>;
}
...
function TopicModal({ topic, onClose, onUpdate }: { topic: StudyTopic; onClose: () => void; onUpdate: (topic: StudyTopic) => void }) {
  ...
  return <Modal label={topic.title} onClose={onClose}><p className="eyebrow">{topic.part === "frontend" ? "Frontend track" : "Full-stack extension"}</p><h2 className="mt-2 text-balance text-3xl font-bold">{topic.title}</h2><p className="mt-1 text-sm text-[var(--muted)]">{topic.breadcrumb}</p><div className="mt-5 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-[var(--subtle)] px-3 py-1">Next review {humanDate(topic.reviewState.dueOn)}</span><span className="rounded-full bg-[var(--subtle)] px-3 py-1 capitalize">{topic.scheduler}</span></div><section className="mt-7"><div className="mb-3 flex items-center justify-between"><h3 className="text-lg font-bold">My notes</h3>{editing ? <div className="flex gap-2"><button className="button-ghost !min-h-9" onClick={() => setEditing(false)}>Cancel</button><button className="button-primary !min-h-9" onClick={save}>Save</button></div> : <button className="button-secondary !min-h-9" onClick={() => setEditing(true)}>Edit</button>}</div>{editing ? <textarea className="field min-h-64 resize-y font-mono text-sm" value={note} onChange={(e) => setNote(e.target.value)} /> : <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-strong)] p-5"><Markdown>{topic.note.markdown || "_No personal notes yet._"}</Markdown></div>}</section>{topic.aiNote && !topic.aiNote.hidden && <section className="mt-7"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="inline-flex items-center gap-2 text-lg font-bold"><Sparkles size={18} className="text-[var(--accent)]" /> AI notes</h3><span className="text-xs text-[var(--muted)]">{topic.aiNote.provider} · {topic.aiNote.model}</span></div>{stale && <p className="mb-3 rounded-xl bg-[var(--subtle)] p-3 text-sm text-[var(--muted)]">Your notes changed after this explanation was generated. It may still be useful.</p>}<div className="rounded-2xl border border-[var(--border)] bg-[var(--accent-soft)] p-5"><p>{topic.aiNote.document.summary}</p><h4 className="mt-4 font-bold">Key points</h4><ul className="mt-2 list-disc pl-5">{topic.aiNote.document.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul><h4 className="mt-4 font-bold">Pitfalls</h4><ul className="mt-2 list-disc pl-5">{topic.aiNote.document.pitfalls.map((point) => <li key={point}>{point}</li>)}</ul></div></section>}</Modal>;
}
